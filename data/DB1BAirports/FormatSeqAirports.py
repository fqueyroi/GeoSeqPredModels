...
Data sets Aviation Support Tables : Master Coordinate found at :
https://www.transtats.bts.gov/Tables.asp?DB_ID=595&DB_Name=Aviation%20Support%20Tables#

Database Name : Aviation Support Tables
Table : Master Coordinate

Needed field : "AirportId", "Latitude", "Longitude"
'''
...
path_cord_file = sys.path[0] + '/MASTER_CORD.csv'
df = pd.read_csv(path_cord_file)
df = df.drop_duplicates(subset="AIRPORT_ID", keep='first')
df = df.drop('Unnamed: 3', 1)
df.to_csv(path_cord_file)
