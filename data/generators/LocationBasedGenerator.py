...
def rbf_kernel(p1, p2, gamma):
	d = dist(p1, p2)
	rbf = math.exp(- (d/math.sqrt(2.))**2 / gamma)
	return rbf

class LocationBasedGenerator(SequenceGenerator.SequenceGenerator):

	def __init__(self, alphabet_size = 100, stop_prob = 0.1,gamma = 0.0001):
		'''
		Parameters:
		-----------
		alphabet_size: int
	...
	def nextSymbol(self, context=[]):
		...
		2 possibilies:
			...
			- one previous locations: a location is picked around the previous
			  one (depending on the value of 'self.gamma')
		'''
		if len(context)==0:
		...
		if len(context)==1:
			...
			for i in range(self.alphabet_size):
				...
				sum_d += rbf_kernel(pc,pi,self.gamma)

			u = random.random()
			cp = 0.
			for i in range(self.alphabet_size):
				pi = self.locations[str(i)]
				prox = rbf_kernel(pc,pi,self.gamma)
				cp += prox/sum_d
				if cp >= u:
					return i
			return -1
...
# gen = LocationBasedGenerator(alphabet_size = 20, stop_prob = 0.1,gamma = 0.01)
